...
//iwaraiiii
    ...
    let pako = await fetchJson(`https://dinka-movie-api.vercel.app/?q=${encodeURIComponent(q)}&apikey=Nadeenxdev2009`)
url = pako.results
    if (!url || url.length === 0) {
        await conn.sendMessage(from, { react: { text: '❌', key: mek.key } });
...
//iwaraiiii
    ...
    const api =
      `https://dinka-movie-api.vercel.app/?url=${encodeURIComponent(q)}&apikey=Nadeenxdev2009`;
...
//iwaraiiii
     ...
     // let searchTitle = cleanText.replace(/[^a-zA-Z0-9 ]/g, "").trim();
      let searchTitle = cleanText.replace(/\s+/g, " ");
        ...
        // API Fetch
        let dl = await fetchJson(`https://nadeen-dinka-pv.vercel.app/api/get-data?q=${encodeURIComponent(searchTitle)}&key=Nadeenxxdevv`);
